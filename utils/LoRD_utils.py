...
def self_search(t,model,sub_net_dict,last_config,finconfig_list,total_config_dict):
    ...
    for i in range(500):
        ...
        sub_config = model.net.sample_active_subnet(t=t, config_list=last_config)
        # sub_config = model.net.sample_active_subnet()
...
def set_stage(model,t,dataset,name):
    if dataset.N_TASKS == 5 :
        model.net.set_stage(t, r=4, task=dataset.N_TASKS)
    elif dataset.N_TASKS == 10:
